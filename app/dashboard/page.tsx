...
import { todayIso, mealSettingsFromRow } from '@/lib/meals';
...
export default async function DashboardPage() {
  ...
  // Admin-only camp meal overview: never fetched for non-admins.
  let campDays: Awaited<ReturnType<typeof getCampOverview>> = [];
  if (isAdmin) {
    ...
    if (supabase) {
      const { data: settingsRow } = await supabase
        ...
        .eq('id', 1)
        .maybeSingle();
      campDays = await getCampOverview(supabase, mealSettingsFromRow(settingsRow), { limit: 5 });
    }
  }
      ...
      {isAdmin && campDays.length > 0 && (
        <div className="mt-6">
          <CampOverview
            ...
            lang={lang}
            compact
            title={mealsT.campMealsTitle}
            desc={mealsT.campMealsDesc}
          />
        </div>
      )}
